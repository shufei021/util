class Local {
    ...
    // 静态方法，用于解析存储的JSON字符串
    static parse = (v: any) => JSON.parse(Local.get(v) ||'{}');

    // 静态方法，用于将对象转换为JSON字符串
    static toStr = JSON.stringify;

    // 静态方法，用于检查是否为字符串类型
    static isStr = (v: any) => typeof v === 'string';

    // 静态方法，用于检查是否为对象类型
    static isObj = (v: any) => Object.prototype.toString.call(v) === '[object Object]';

    // 静态方法，用于检查是否为数字类型
    static isNum = (n: any) => Number.isInteger(n) && /^[+-]?\d+(\.\d+)?$/.test(String(n));

    // 静态方法，用于检查localStorage中是否包含指定键名的项
    static hasOwn = (a: string) => Object.prototype.hasOwnProperty.call(Local.local, a);
...
export const local = new Local(); // 导出实例
