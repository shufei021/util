...
export const round:Function = function (n: any, dight: number | undefined): number | string{
    // 将输入的数字转换为字符串
    n = Number(n).toString();
    ...
    if (n.includes('e')) {
        ...
        if (coefficient.includes('.')) {
            const arr:string[] = coefficient.split('.');
            const dotLen:number = arr[1].length;
        ...
        if (exponent > 0) {
            // 构造补零部分
            const end:string = '0'.repeat(exponent);
            // 合并系数和补零部分
            const rt:string = coefficient + end;
            // 如果指定保留小数位数，则返回带小数的结果
            if (dight) return rt + '.' + '0'.repeat(dight);
            // 如果补零位数大于系数位数，直接返回结果
            if (exponent > coefficient.length || dight === undefined) return rt;
            // 补零位数小于等于系数位数，截取结果
